            ...
            >
              <div className="flex justify-between items-center">
                <span className="font-bold text-white text-sm flex items-center gap-2">
                  <span className="w-5 h-5 rounded-full bg-amber-500/10 text-amber-400 font-bold flex items-center justify-center text-[11px] border border-amber-500/30">
                    {idx + 1}
                  </span>
                  {item.drill}
                </span>
                <span className="px-2.5 py-0.5 rounded-full bg-neutral-900 text-amber-300 font-mono text-[11px] border border-neutral-800">
                  {item.setsReps}
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] text-neutral-300 pt-1 border-t border-neutral-900">
                <div>
                  <span className="text-neutral-400 font-medium">Articulación Diana:</span> {item.targetJoint}
                </div>
                <div>
                  <span className="text-neutral-400 font-medium">Objetivo Biomecánico:</span> {item.objective}
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="space-y-3">
          {exercise.postStretchingDetail?.map((item, idx) => (
            ...
            >
              <div className="flex justify-between items-center">
                <span className="font-bold text-white text-sm flex items-center gap-2">
                  <span className="w-5 h-5 rounded-full bg-cyan-500/10 text-cyan-400 font-bold flex items-center justify-center text-[11px] border border-cyan-500/30">
                    {idx + 1}
                  </span>
                  {item.stretch}
                </span>
                <span className="px-2.5 py-0.5 rounded-full bg-cyan-500/10 text-cyan-300 font-mono text-[11px] border border-cyan-500/20">
                  {item.duration}
                </span>
              </div>
